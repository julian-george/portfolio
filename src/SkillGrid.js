...
class SkillGrid extends React.Component {
  ...
  render() {
    ...
    let skillComponents = this.props.skills.map((s, index) => (
      <Skill
        toggle={() => {
          this.openModal(s);
        }}
          ...
          <div id="modalHead">
            ...
            <div id="modalInfo">
              ...
              <div id="modalExperience">
                ...
                <span>
                  {this.experienceLabels[this.props.skill.experience - 1]}
                </span>
              </div>
            </div>
          </div>
          <hr />
          <div id="modalBody">{this.props.skill.description}</div>
        </div>
      </Fade>
    );
  }
...
class ExperienceBubble extends React.Component {
  constructor(props) {
    super(props);
    this.bubbleColors = ["green", "orange", "red", "blue", "purple"];
  }
  render() {
    return (
      <div
        className="expBubble"
        style={{ backgroundColor: this.bubbleColors[this.props.level - 1] }}
      >

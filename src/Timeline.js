...
export default function Timeline(props) {
  ...
  const maxDisp = -(
    moveAmount * (projects.length - 3) +
    props.timeline.bubbleSize +
    60
  );
  ...
  let leftArrow = (
    <svg height="60" width="40">
      <polyline
        ...
        style={{ stroke: leftActive ? activeHue : inactiveHue }}
        fill="none"
        strokeWidth="6"
      />
    </svg>
  ...
  let rightArrow = (
    <svg height="60" width="40">
      <polyline
        ...
        style={{ stroke: rightActive ? activeHue : inactiveHue }}
        fill="none"
        strokeWidth="6"
      />
    </svg>
  ...
  return (
    <div id="projectlist">
      <div
        ...
        style={{ cursor: leftActive ? "pointer" : "auto" }}
        onClick={() => {
          move(moveAmount);
        }}
      >
      ...
      <div id="innerProjects">
        <animated.div id="projectsContainer" style={dispSpring}>
          {" "}
          {projects}{" "}
        </animated.div>
        <div id="timeline"></div>
      ...
      <div
        id="navRight"
        className="timelineNav"
        style={{ cursor: rightActive ? "pointer" : "auto" }}
        onClick={() => {
          move(moveAmount * -1);
        }}
      >
      ...
      <div
        ...
        style={{
          ...
          backgroundColor: this.isCurrent
            ? this.state.hovering
              ? "hsl(" + this.props.hue + ",95%,25%)"
              : "hsl(" + this.props.hue + ",75%,40%)"
            : "white",
        }}
      >
        <div
        ...
        >
          ...
          <div className="projectDate">
            {project.hasOwnProperty("time") ? project.time : ""}
          </div>
        </div>
        <div
        ...
        <Fade in={this.props.active}>
          <div className="projectInfo">
            ...
            <div className="projectInfoBody">
              <div className="projectInfoSummary">{project.summary}</div>
              <div className="projectInfoSkills">
                {project.skills.map((skill, index) => {
                  return (
                    <SkillPill text={skill} key={index} hue={this.props.hue} />
                  );
                })}
              </div>
              <div className="projectInfoTakeaways">
                ...
                <ul>
                  {project.takeaways.map((take, index) => {
                    return <li key={index}>{take}</li>;
                  })}
                </ul>
              </div>
              {project.link != "" ? (
                <div className="projectInfoLink">
                  <a href={project.link}> Visit Here </a>
                </div>
              ) : (
                ""
              )}
...
class SkillPill extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="skillPill" style={{ backgroundColor: pillColor }}>
        {this.props.text}
      </div>
    );
  }
